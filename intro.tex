...
\begin{document}
	...
	where $x=\dfrac{a}{b}$. This can be generalized to the following result.
		\begin{theorem}[\index{Arithmetic-Geometric Inequality}Arithmetic-Geometric Inequality]\label{thm:amgm}
	...
	Note the following.
		\begin{align*}
			\dfrac{a+b}{2}
				& \geq\sqrt{ab}
		\end{align*}
	Now, $\frac{a+b}{2}$ is the \index{arithmetic mean}\emph{arithmetic mean} of $a$ and $b$. On the right side, $\sqrt{ab}$ is the \index{geometric mean}\emph{geometric mean} of $a$ and $b$. So the inequality states that the arithmetic mean of two positive real numbers is greater than or equal to their geometric mean. We can also rewrite it as the following.
		\begin{align*}
		...
		\end{align*}
	$\dfrac{2ab}{a+b}$ is the \index{harmonic mean}\emph{harmonic mean} of $a$ and $b$. So, this form of the inequality states that the geometric mean is larger than the harmonic mean. This can be extended for three variables.
		\begin{align*}
			\dfrac{a+b+c}{3}
	...
	So $y_n$ is decreasing. Using the two problems above, show that $2<e<4$. In fact, we can prove the following using elementary means
		...
		\begin{problem}
			...
			Equality occurs only for $n=1$.
				...
				\end{solution}
		\end{problem}
	Let us again go back to the mother of inequality. Setting $x_1=a-b,x_2=b-c,x_3=c-a$ in \hyperref[ineq:extendedmother]{\ref{ineq:extendedmother}},
		\begin{align*}
			(a-b)^2+(b-c)^2+(c-a)^2
				& \geq0\\
	...
	The last inequality immediately follows from the assumption that $a-c\geq0,a-b\geq0,(b-c)^2\geq0$. Also, note that we did not actually require the condition $a\geq b\geq c$. Just assuming $a=\max(a,b,c)$ was enough in this case to claim that the inequality holds. 
		\begin{theorem}[\index{Nesbitt's inequality}Nesbitt's inequality]\label{thm:nesbitt}
	...
	First, we will try to \emph{familiarize} the expression on the left side.
		...
		\begin{proof}[\index{Variable reduction}Variable reduction proof]
			...
			We additionally have $mb_n\leq s_nb_n\leq Mb_n$. Summing these inequalities together,
				\begin{align*}
					m(b_1-b_2+b_2-b_3+\ldots+b_{n-1}-b_n)+mb_n
						& \leq s_1(b_1-b_2)+\ldots+s_{n-1}(b_{n-1}-b_n)+s_nb_n\\
	...
	Try the following problems as warm up exercises.
		...
		\end{problem}
	The first inequality in this problem is also known as the \index{triangle inequality}\emph{triangle inequality}.
		\begin{problem}
			Prove \index{Jordan's inequality}\emph{Jordan's inequality}.
				\begin{align*}
					\dfrac{2}{\pi}
		...
		\begin{problem}
			Prove \index{Redheffer's inequality}\emph{Redheffer's inequality}.
				\begin{align*}
					\dfrac{\sin{\theta}}{\theta}
