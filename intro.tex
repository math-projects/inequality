...
\begin{document}
	...
	Setting $x_1=a-b,x_2=b-c,x_3=c-a$ in \hyperref[ineq:extendedmother]{inequality $\ddagger$\ref{ineq:extendedmother}},
		\begin{align*}
			(a-b)^2+(b-c)^2+(c-a)^2
				& \geq0\\
			...
			We write the arithmetic-harmonic mean inequality for $u,v,w$ as below.
				\begin{align*}
					...
					\iff(u+v+w)\left(\dfrac{1}{u}+\dfrac{1}{v}+\dfrac{1}{w}\right)
						& \geq9
				\end{align*}
			Using this on \hyperref[eqn:nesbittf]{equation $\ddagger$\ref{eqn:nesbittf}},
				\begin{align*}
					S
	...
	Before we go into any theory or method of solving problems, here are some basic tactics which are used very often for solving problems. Let $x,y,z>0$ be real numbers.
		\begin{enumerate}[(i)]
			...
			\item Check if you can get some familiar expressions with some basic manipulations such as making the numerator or denominator equal. For example, see the transformation in \hyperref[eqn:nesbittf]{equation $\ddagger$\ref{eqn:nesbittf}}. This often helps us get a clue on what to do with the inequality.
			...
			\item Can you reduce the number of variables without any assumption? See another proof of Nesbitt's inequality below.
		\end{enumerate}
	For demonstration purposes, let us prove $a^2+b^2+c^2\geq ab+bc+ca$ exploiting symmetry. Note that if we let
		\begin{align*}
			f(a,b,c)
				& = a^2+b^2+c^2-ab-bc-ca
		\end{align*}
	then $f(a,b,c)$ is \textit{symmetric} on $a,b,c$. We can verify this by the fact that $f(a,b,c)=f(b,a,c)=f(c,a,b)$ and so on. So, without loss of generality, we assume that $a\geq b\geq c$. Then see the following.
		\begin{align*}
			...
			(a-c)(a-b)+(b-c)^2
				& \geq0
		\end{align*}
	The last inequality immediately follows from the assumption that $a-c\geq0,a-b\geq0,(b-c)^2\geq0$. Also, note that we did not actually require the condition $a\geq b\geq c$. Just assuming $a=\max(a,b,c)$ was enough in this case to claim that the inequality holds. Like we mentioned above, we will prove Nesbitt's inequality once again.
		\begin{proof}[Proof of Nesbitt's inequality with variable reduction]
