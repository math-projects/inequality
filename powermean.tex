...
\begin{document}
	...
	In general, the \index{weighted power mean}\textit{weighted power mean of order} $r$ is
		...
		\end{align*}
	If the context is clear on what the weights are, then we may omit the weight from the notation and simply write $\mathfrak{M}_{r}(\mathbf{a})$. Let us call $\omega$ the \textit{weight vector}. We can convert almost any vector into a weight vector. If $\tau=(t_{1},\ldots,t_{n})$ is an arbitrary vector not all elements zero, then
		\begin{align*}
			\omega
