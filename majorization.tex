...
\begin{document}
	...
	If $\mathbf{a}$ be a vector with $n$ elements. We write
		...
		\end{align*}
	We will explain it a little more. Say, we want to find the symmetric polynomials of the type $x^{3}y$ for the variables $x,y,z$. Write it as $x^{3}y^{1}z^{0}$. Now, we fix the powers $3,1,0$ in their respective places and let $x,y,z$ run through the permutations. Then we get
		\begin{align*}
	...
	where $S_{k}$ is the sum of products of $a_{1},\ldots,a_{n}$ taken $k$ at the same time. If $S_{k}$ is the coefficient of $x^{n-k}$ in the expansion, then
		...
		\begin{theorem}[Newton's inequality]\label{thm:newton}
		...
		\begin{theorem}[Maclaurin's inequality]\label{thm:maclaurin}
